    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-muted-foreground mb-6">
          You don't have an active programme enrollment yet.
        </p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {unlockedModules.length > 0 && (
          <button
            onClick={() => navigate(`/modules/${unlockedModules[unlockedModules.length - 1].id}`)}
            className="p-5 rounded-xl border border-primary/20 bg-primary/5 text-left hover:bg-primary/10 transition-colors group"
          >
            <BookOpen className="w-5 h-5 text-primary mb-3" />
            <p className="font-medium text-sm mb-1">Continue Learning</p>
            <p className="text-xs text-muted-foreground">
              Day {unlockedModules[unlockedModules.length - 1].day_number}: {unlockedModules[unlockedModules.length - 1].title}
            </p>
        ...
        {nextModule && (
          <div className="p-5 rounded-xl border border-border/50 bg-card/30">
            ...
            <p className="text-xs text-muted-foreground">
              Day {nextModule.day_number}: {nextModule.title}
            </p>
            <p className="text-xs text-primary mt-2">
              {new Date(nextModule.unlock_date).toLocaleDateString("en-GB", {
                weekday: "short",
                day: "numeric",
                month: "short",
              })}
