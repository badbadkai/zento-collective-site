...
import type { Module, Enrollment } from "@/shared/types/database";
import { Lock, Check, BookOpen, Coffee } from "lucide-react";
...
export default function PortalModules() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      // Get modules and submissions in parallel
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-4 p-4 rounded-xl border text-left transition-all ${
                isUnlocked
                  ? "border-border/50 bg-card/30 hover:bg-card/60 hover:border-primary/30 cursor-pointer"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {mod.is_rest_day && (
                    <span className="text-xs bg-muted px-1.5 py-0.5 rounded">Rest Day</span>
                  )}
                </div>
              ...
              {/* Unlock date for locked modules */}
              {!isUnlocked && (
                <span className="text-xs text-muted-foreground shrink-0">
                  {new Date(mod.unlock_date).toLocaleDateString("en-GB", {
                    day: "numeric",
                    month: "short",
                  })}
                </span>
