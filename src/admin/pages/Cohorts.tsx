...
export default function AdminCohorts() {
  ...
  const handleSave = async () => {
    if (editing) {
      ...
      await supabase.from("cohorts").insert(form);
    }
    resetForm();
    fetchCohorts();
  };
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2 space-y-2">
              ...
              <Input
                ...
                placeholder="e.g. Cohort 1 — March 2026"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, max_seats: parseInt(e.target.value) || 15 })}
              />
            </div>
            <div className="space-y-2">
              <Label>Status</Label>
              <select
                value={form.status}
                onChange={(e) => setForm({ ...form, status: e.target.value as Cohort["status"] })}
          ...
          cohorts.map((cohort) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColor(cohort.status)}`}>
                  {cohort.status}
                </span>
                <Button variant="ghost" size="icon" onClick={() => startEdit(cohort)}>
